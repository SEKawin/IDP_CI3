<!--modal Form -->
<div class="modal fade" id="modal_idp_plan_form_apporval_gm" role="dialog">
    <div id="modal_dialog" class="modal-dialog modal-dialog-scrollable" style="max-width: 100%;">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal_title_idp_plan_form_apporval_gm">Form</h3>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body form" id="append-modal-gm">
            </div> <!-- modal-body form -->
...
<div id="hidden-modal-body-gm" style="display: none;">
    <form action="#" id="idp_plan_form_apporval_gm" class="form-horizontal">
...
<script type="text/javascript">
    ...
    function idp_form_approval_gm(idp_form_id) {
        $('#append-modal-gm').html($('#hidden-modal-body-gm').html());
        $('body').find('#idp_plan_form_apporval_gm')[0].reset(); // reset form on modals
        $('.form-group').removeClass('has-error'); // clear error class
        $('#modal_idp_plan_form_apporval_gm').modal('show'); // show bootstrap modal
        $('.modal_title_idp_plan_form_apporval_gm').html('<p>แผนพัฒนาบุคลากรรายบุคคล</p>'); // Set Title to Bootstrap modal title
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $(function() {
                    $(".choice_gm").change(function() {
                        if ($(this).val() == 1) {
                            $('#remark_gm').val('').change();
                            $(".txtbox_gm").hide('slow');
                        } else {
                ...
                //  Core Competency 
                $('#modal_idp_plan_form_apporval_gm #tbody_CC').html('');
                $.each(data.idp_cc, function(key, value) {
                    $('#modal_idp_plan_form_apporval_gm #tbody_CC').append(`
						<tr>
							<td>${[value.no]}</td>
                ...
                // Core Competency 
                // Manage Competency 
                $('#modal_idp_plan_form_apporval_gm #tbody_MC').html('');
                if (data.idp_mc == null) {
                    $('#TopicMC').hide();
                    $('#tbody_MC').hide();
                } else {
                    $.each(data.idp_mc, function(key, value) {
                        $('#TopicMC').show();
                        $('#modal_idp_plan_form_apporval_gm #tbody_MC').append(`
						<tr>
							<td>${[value.no]}</td>
							<td>${[value.competency]}</td>
							<td>${[value.learning]}</td>
                ...
                // ความรู้ Knowledge
                $('#modal_idp_plan_form_apporval_gm #tbody_Know').html('');
                $.each(data.idp_know, function(key, value) {
                    $('#modal_idp_plan_form_apporval_gm #tbody_Know').append(`
						<tr>
							<td>${[value.no]}</td>
							<td>${[value.competency]}</td>
							<td>${[value.learning]}</td>
                ...
                // Skill 
                $('#modal_idp_plan_form_apporval_gm #tbody_Skill').html('');
                $.each(data.idp_skill, function(key, value) {
                    $('#modal_idp_plan_form_apporval_gm #tbody_Skill').append(`
						<tr>
							<td>${[value.no]}</td>
							<td>${[value.competency]}</td>
							<td>${[value.learning]}</td>
                ...
                // Personal Attributes
                $('#modal_idp_plan_form_apporval_gm #tbody_PerAtt').html('');
                $.each(data.idp_perattr, function(key, value) {
                    $('#modal_idp_plan_form_apporval_gm #tbody_PerAtt').append(`
						<tr>
							<td>${[value.no]}</td>
							<td>${[value.competency]}</td>
							<td>${[value.learning]}</td>
                ...
                $.each(data.data_status, function(key, value) {
                    if (value.mgr_status == 0) {
                        $('#modal_idp_plan_form_apporval_gm #mgr_status').html(`<span style="font-size: 15px" class=" badge badge-secondary">รอดำเนินการ</span>`);
                    } else if (value.mgr_status == 1) {
                        $('#modal_idp_plan_form_apporval_gm #mgr_status').html(`<label>ผลสถานะ</label><span style="font-size: 15px" class=" badge badge-success">อนุมัติ</span>`);
                    } else if (value.mgr_status == 2) {
                        $('#modal_idp_plan_form_apporval_gm #mgr_status').html(`<label>ผลสถานะ</label><span style="font-size: 15px" class=" badge badge-danger">ไม่อนุมัติ</span>`);
                    }

                    if (value.gm_status == null || value.gm_status == 0) {
                        $('#modal_idp_plan_form_apporval_gm #gm_status').html(`
                            <div class="input-group mb-3">
                                <div class="input-group-prepend">
                                    <span class="input-group-text" id="basic-addon">ผลสถานะ</span>
                                </div>
                                <select class="choice_gm custom-select" id="inputGroupSelect" name="gm_status">
                                    <option selected>เลือก...</option>
                                    <option value="1">อนุมัติ</option>
                                    <option value="2">ไม่อนุมัติ</option>
                            </select>
                            <button type="button" class="btn btn-sm btn-primary" id="btnApprovalGm" onclick="approval_form_gm()">ยืนยัน</button>
                            </div>
                        `);
                    } else if (value.gm_status == 1) {
                        $('#modal_idp_plan_form_apporval_gm #gm_status').html(`<span style="font-size: 15px" class=" badge badge-success">อนุมัติ</span>`);
                    } else if (value.gm_status == 2) {
                        $('#modal_idp_plan_form_apporval_gm #gm_status').html(`<span style="font-size: 15px" class=" badge badge-danger">ไม่อนุมัติ</span>`);
                    }

                    if (value.mgr_hrd_status == null || value.mgr_hrd_status == 0) {
                        $('#modal_idp_plan_form_apporval_gm #mgr_hrd_status').html(`<span style="font-size: 15px" class=" badge badge-secondary">รอดำเนินการ</span>`);

                    } else if (value.mgr_hrd_status == 1) {
                        $('#modal_idp_plan_form_apporval_gm #mgr_hrd_status').html(`<span style="font-size: 15px" class=" badge badge-success">อนุมัติ</span>`);
                    } else if (value.mgr_hrd_status == 2) {
                        $('#modal_idp_plan_form_apporval_gm #mgr_hrd_status').html(`<span style="font-size: 15px" class=" badge badge-danger">ไม่อนุมัติ</span>`);
                    }

                    if (value.od_status == null || value.od_status == 0) {
                        $('#modal_idp_plan_form_apporval_gm #od_status').html(`<span style="font-size: 15px" class=" badge badge-secondary">รอดำเนินการ</span>`);
                    } else if (value.od_status == 1) {
                        $('#modal_idp_plan_form_apporval_gm #od_status').html(`<span style="font-size: 15px" class=" badge badge-success">อนุมัติ</span>`);
                    } else if (value.od_status == 2) {
                        $('#modal_idp_plan_form_apporval_gm #od_status').html(`<span style="font-size: 15px" class=" badge badge-danger">ไม่อนุมัติ</span>`);
                    }
                })
    ...
    function approval_form_gm() {
        $('#btnApprovalGm').text('กำลังอนุัมติแบบฟอร์ม...'); //change button text
        $('#btnApprovalGm').attr('disabled', true); //set button disable

        var formdata = $('#idp_plan_form_apporval_gm').serialize();

        $.ajax({
            type: 'POST',
            url: "<?php echo site_url('idp_form/idp_approval_form_gm') ?>",
            data: formdata,
            dataType: 'JSON',
            success: function(data) {
                $('#tbl_approval_gm').DataTable().ajax.reload(null, false);
                $('#idp_plan_form_apporval_gm')[0].reset(); // reset form on modals
                $('#modal_idp_plan_form_apporval_gm').modal('hide');
                alert('คุณทำการอนุมัติเรียบร้อยแล้ว');

                $('#btnApprovalGm').text('ยืนยัน'); //change button text
                $('#btnApprovalGm').attr('disabled', false); //set button enable
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert(messenger);
            }
