<!--modal Form -->
<div class="modal fade" id="modal_idp_plan_form_apporval_mgr" role="dialog">
    <div id="modal_dialog" class="modal-dialog modal-dialog-scrollable" style="max-width: 100%;">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal_title_idp_plan_form_apporval_mgr">Form</h3>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body form" id="append-modal-mgr">
            </div> <!-- modal-body form -->
...
<div id="hidden-modal-body-mgr" style="display: none;">
    <form action="#" id="idp_plan_form_apporval_mgr" class="form-horizontal">
...
<script type="text/javascript">
    ...
    function idp_form_approval_mgr(idp_form_id) {
        $('#append-modal-mgr').html($('#hidden-modal-body-mgr').html());
        $('body').find('#idp_plan_form_apporval_mgr')[0].reset(); // reset form on modals
        $('.form-group').removeClass('has-error'); // clear error class
        $('#modal_idp_plan_form_apporval_mgr').modal('show'); // show bootstrap modal
        $('.modal_title_idp_plan_form_apporval_mgr').html('<p>แผนพัฒนาบุคลากรรายบุคคล</p>'); // Set Title to Bootstrap modal title
        ...
        $.ajax({
            ...
            success: function(data) {
                $(function() {
                    $(".choice_mgr").change(function() {
                        if ($(this).val() == 1) {
                            $('#remark_mgr').val('').change();
                            $(".txtbox_mgr").hide('slow');
                        } else {

                            $(".txtbox_mgr").show('slow');
                        }
                    });
                });
                ...
                //  Core Competency 
                $('#modal_idp_plan_form_apporval_mgr #tbody_CC').html('');
                $.each(data.idp_cc, function(key, value) {
                    $('#modal_idp_plan_form_apporval_mgr #tbody_CC').append(`
						<tr>
							<td>${[value.no]}</td>
                ...
                // Core Competency 
                // Manage Competency 
                $('#modal_idp_plan_form_apporval_mgr #tbody_MC').html('');
                if (data.idp_mc == null) {
                    $('#TopicMC').hide();
                    $('#tbody_MC').hide();
                } else {
                    $.each(data.idp_mc, function(key, value) {
                        $('#TopicMC').show();
                        $('#modal_idp_plan_form_apporval_mgr #tbody_MC').append(`
						<tr>
							<td>${[value.no]}</td>
							<td>${[value.competency]}</td>
							<td>${[value.learning]}</td>
                ...
                // ความรู้ Knowledge
                $('#modal_idp_plan_form_apporval_mgr #tbody_Know').html('');
                $.each(data.idp_know, function(key, value) {
                    $('#modal_idp_plan_form_apporval_mgr #tbody_Know').append(`
						<tr>
							<td>${[value.no]}</td>
							<td>${[value.competency]}</td>
							<td>${[value.learning]}</td>
                ...
                // Skill 
                $('#modal_idp_plan_form_apporval_mgr #tbody_Skill').html('');
                $.each(data.idp_skill, function(key, value) {
                    $('#modal_idp_plan_form_apporval_mgr #tbody_Skill').append(`
						<tr>
							<td>${[value.no]}</td>
							<td>${[value.competency]}</td>
							<td>${[value.learning]}</td>
                ...
                // Personal Attributes
                $('#modal_idp_plan_form_apporval_mgr #tbody_PerAtt').html('');
                $.each(data.idp_perattr, function(key, value) {
                    $('#modal_idp_plan_form_apporval_mgr #tbody_PerAtt').append(`
						<tr>
							<td>${[value.no]}</td>
							<td>${[value.competency]}</td>
							<td>${[value.learning]}</td>
                ...
                $.each(data.data_status, function(key, value) {
                    if (value.mgr_status == 0) {
                        $('#modal_idp_plan_form_apporval_mgr #mgr_status').html(`
                            <div class="input-group mb-3">
                                <div class="input-group-prepend">
                                    <span class="input-group-text" id="basic-addon">ผลสถานะ</span>
                                </div>
                                <select class="choice_mgr custom-select" id="inputGroupSelect" name="mgr_status">
                                    <option selected>เลือก...</option>
                                    <option value="1">อนุมัติ</option>
                                    <option value="2">ไม่อนุมัติ</option>
                            </select>
                            <button type="button" class="btn btn-sm btn-primary" id="btnApprovalMgr" onclick="approval_form_mgr()">ยืนยัน</button>
                            </div>
                        `);
                    } else if (value.mgr_status == 1) {
                        $('#modal_idp_plan_form_apporval_mgr #mgr_status').html(`<label>ผลสถานะ</label><span style="font-size: 15px" class=" badge badge-success">อนุมัติ</span>`);
                    } else if (value.mgr_status == 2) {
                        $('#modal_idp_plan_form_apporval_mgr #mgr_status').html(`<label>ผลสถานะ</label><span style="font-size: 15px" class=" badge badge-danger">ไม่อนุมัติ</span>`);
                    }

                    if (value.gm_status == null || value.gm_status == 0) {
                        $('#modal_idp_plan_form_apporval_mgr #gm_status').html(`<span style="font-size: 15px" class=" badge badge-secondary">รอดำเนินการ</span>`);
                    } else if (value.gm_status == 1) {
                        $('#modal_idp_plan_form_apporval_mgr #gm_status').html(`<span style="font-size: 15px" class=" badge badge-success">อนุมัติ</span>`);
                    } else if (value.gm_status == 2) {
                        $('#modal_idp_plan_form_apporval_mgr #gm_status').html(`<span style="font-size: 15px" class=" badge badge-danger">ไม่อนุมัติ</span>`);
                    }

                    if (value.mgr_hrd_status == null || value.mgr_hrd_status == 0) {
                        $('#modal_idp_plan_form_apporval_mgr #mgr_hrd_status').html(`<span style="font-size: 15px" class=" badge badge-secondary">รอดำเนินการ</span>`);

                    } else if (value.mgr_hrd_status == 1) {
                        $('#modal_idp_plan_form_apporval_mgr #mgr_hrd_status').html(`<span style="font-size: 15px" class=" badge badge-success">อนุมัติ</span>`);
                    } else if (value.mgr_hrd_status == 2) {
                        $('#modal_idp_plan_form_apporval_mgr #mgr_hrd_status').html(`<span style="font-size: 15px" class=" badge badge-danger">ไม่อนุมัติ</span>`);
                    }

                    if (value.od_status == null || value.od_status == 0) {
                        $('#modal_idp_plan_form_apporval_mgr #od_status').html(`<span style="font-size: 15px" class=" badge badge-secondary">รอดำเนินการ</span>`);
                    } else if (value.od_status == 1) {
                        $('#modal_idp_plan_form_apporval_mgr #od_status').html(`<span style="font-size: 15px" class=" badge badge-success">อนุมัติ</span>`);
                    } else if (value.od_status == 2) {
                        $('#modal_idp_plan_form_apporval_mgr #od_status').html(`<span style="font-size: 15px" class=" badge badge-danger">ไม่อนุมัติ</span>`);
                    }
                })
    ...
    function approval_form_mgr() {
        $('#btnApprovalMgr').text('กำลังอนุัมติแบบฟอร์ม...'); //change button text
        $('#btnApprovalMgr').attr('disabled', true); //set button disable

        var formdata = $('#idp_plan_form_apporval_mgr').serialize();

        $.ajax({
            type: 'POST',
            url: "<?php echo site_url('idp_form/idp_approval_form_mgr') ?>",
            data: formdata,
            dataType: 'JSON',
            success: function(data) {
                $('#tbl_approval_mgr').DataTable().ajax.reload(null, false);
                $('#idp_plan_form_apporval_mgr')[0].reset(); // reset form on modals
                $('#modal_idp_plan_form_apporval_mgr').modal('hide');
                alert('คุณทำการอนุมัติเรียบร้อยแล้ว');

                $('#btnApprovalMgr').text('ยืนยัน'); //change button text
                $('#btnApprovalMgr').attr('disabled', false); //set button enable
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert(messenger);
            }
