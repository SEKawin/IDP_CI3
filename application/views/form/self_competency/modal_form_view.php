<div class="modal fade" id="modal_form_view_self_competency" role="dialog">
	<div class="modal-dialog modal-dialog-scrollable modal-xl">
		<div class="modal-content">
			<!-- Modal Header -->
			<div class="modal-header">
				<h5 class="modal-title-view-form">แบบประเมินช่องว่างความสามารถของตนเอง (Self-Competency Gap Assessment) ประจำปี <label name="years"></label></h5>
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			</div>
			<!-- Modal body -->
			<div class="modal-body">
				<form action="#" id="form_view_self_competency" class="form-horizontal">
					<div class="form-row">
...
<script type="text/javascript">
	...
	function view_self_competency_form(form_id) {
		$('#form_view_self_competency')[0].reset(); // reset form on modals
		$('.form-group').removeClass('has-error'); // clear error class
		$('.help-block').empty(); // clear error string
		$('#modal_form_view_self_competency').modal('show'); // show bootstrap modal
		...
		$.ajax({
			...
			success: function(data) {
				...
				$.each(data.core_competency, function(key, value) {
					$('#modal_form_view_self_competency #firstCcTr').html('');
					for (var i = 0; i < value.length; i++) {
						$('#modal_form_view_self_competency #firstCcTr').append(`<tr>
                        <td>${value[i].no}</td>
                        <td>${value[i].code}</td>
				...
				$.each(data.manage_competency, function(key, value) {
					if (value == null) {
						$('#modal_form_view_self_competency #firstMcTr').html('');

					} else {
						$('#modal_form_view_self_competency #firstMcTr').html('');
						for (var i = 0; i < value.length; i++) {
							$('#modal_form_view_self_competency #firstMcTr').append(`<tr>
								<td>${value[i].no}</td>
								<td>${value[i].code}</td>
								<td>${value[i].competency}</td>
								<td>${value[i].expected}</td>
				...
				$.each(data.knowledge, function(key, value) {
					$('#modal_form_view_self_competency #firstKnowTr').html('');
					for (var i = 0; i < value.length; i++) {
						$('#modal_form_view_self_competency #firstKnowTr').append(`<tr>
                        <td>${value[i].no}</td>
                        <td>${value[i].code}</td>
                        <td>${value[i].competency}</td>
                        <td>${value[i].expected}</td>
				...
				$.each(data.skill, function(key, value) {
					$('#modal_form_view_self_competency #firstSkillTr').html('');
					for (var i = 0; i < value.length; i++) {
						$('#modal_form_view_self_competency #firstSkillTr').append(`<tr>
                        <td>${value[i].no}</td>
                        <td>${value[i].code}</td>
                        <td>${value[i].competency}</td>
                        <td>${value[i].expected}</td>
				...
				$.each(data.personal_attributes, function(key, value) {
					$('#modal_form_view_self_competency #firstPerAttrTr').html('');
					for (var i = 0; i < value.length; i++) {
						$('#modal_form_view_self_competency #firstPerAttrTr').append(`<tr>
                        <td>${value[i].no}</td>
                        <td>${value[i].code}</td>
                        <td>${value[i].competency}</td>
                        <td>${value[i].expected}</td>
