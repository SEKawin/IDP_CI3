<div class="modal fade" id="modal_form_approval_self_competency" role="dialog">
    <div class="modal-dialog modal-dialog-scrollable modal-xl">
        <div class="modal-content">
            <!-- Modal Header -->
            <div class="modal-header">
                <h5 class="modal-title-approval-form">แบบประเมินช่องว่างความสามารถของตนเอง (Self-Competency Gap Assessment) ประจำปี <label name="years"></label></h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <!-- Modal body -->
            <div class="modal-body">
                <form action="#" id="form_approval_self_competency" class="form-horizontal">
                    ...
                    <div class="table-sm table-responsive">
                        ...
                        <div class="input-group form-group col-md-4" id="approval_form">
                            ...
                            <button type="button" class="btn btn-sm btn-primary" id="btn_approval_form" onclick="approval_form()">ยืนยัน</button>
                        </div>
                        <hr class=" w-100 ml-0 text-center">

                        <div class="form-group col-md-4" style="display: none" id="approval_status">
                            <h6 colspan="3" style="padding-left: 1.5em;">
                                <label><b>ผู้อนุัมติต้นสังกัด</b></label>
                                <label name="form_mgr_name"></label><br>
...
<script type="text/javascript">
    var messenger = 'ไม่สามารถกดอนุมัติ โปรดแจ้งเจ้าหน้าที่รับผิดชอบระบบนี้ เบอร์ 1104 หรือ ช่องทางแชทของ MS TEAM (kawin_a@thaisummit-harness.co.th';

    function form_approval_mgr(form_id) {

        $('#form_approval_self_competency')[0].reset(); // reset form on modals
        $('.form-group').removeClass('has-error'); // clear error class
        $('.help-block').empty(); // clear error string
        $('#modal_form_approval_self_competency').modal('show'); // show bootstrap modal
        ...
        $.ajax({
            url: "<?php echo site_url('Self_competency/view_self_competency_form') ?>/" + form_id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {

                $.each(data.core_competency, function(key, value) {
                    $('#modal_form_approval_self_competency #firstCcTr').html('');
                    for (var i = 0; i < value.length; i++) {
                        $('#modal_form_approval_self_competency #firstCcTr').append(`<tr>
                        <td>${value[i].no}</td>
                        <td>${value[i].code}</td>
                ...
                $.each(data.manage_competency, function(key, value) {
                    if (value != null) {
                        $('#modal_form_approval_self_competency #firstMcTr').html('');
                        for (var i = 0; i < value.length; i++) {
                            $('#modal_form_approval_self_competency #firstMcTr').append(`<tr>
								<td>${value[i].no}</td>
								<td>${value[i].code}</td>
								<td>${value[i].competency}</td>
								<td>${value[i].expected}</td>
                ...
                $.each(data.knowledge, function(key, value) {
                    $('#modal_form_approval_self_competency #firstKnowTr').html('');
                    for (var i = 0; i < value.length; i++) {
                        $('#modal_form_approval_self_competency #firstKnowTr').append(`<tr>
                        <td>${value[i].no}</td>
                        <td>${value[i].code}</td>
                        <td>${value[i].competency}</td>
                        <td>${value[i].expected}</td>
                ...
                $.each(data.skill, function(key, value) {
                    $('#modal_form_approval_self_competency #firstSkillTr').html('');
                    for (var i = 0; i < value.length; i++) {
                        $('#modal_form_approval_self_competency #firstSkillTr').append(`<tr>
                        <td>${value[i].no}</td>
                        <td>${value[i].code}</td>
                        <td>${value[i].competency}</td>
                        <td>${value[i].expected}</td>
                ...
                $.each(data.personal_attributes, function(key, value) {
                    $('#modal_form_approval_self_competency #firstPerAttrTr').html('');
                    for (var i = 0; i < value.length; i++) {
                        $('#modal_form_approval_self_competency #firstPerAttrTr').append(`<tr>
                        <td>${value[i].no}</td>
                        <td>${value[i].code}</td>
                        <td>${value[i].competency}</td>
                        <td>${value[i].expected}</td>
    ...
    function approval_form() {
        $('#btn_approval').text('กำลังทำการยืนยัน...'); //change button text
        $('#btn_approval').attr('disabled', true); //set button disable

        var formdata = $('#form_approval_self_competency').serialize();

        $.ajax({
            type: 'POST',
            url: "<?php echo site_url('Self_competency/approval_form_mgr') ?>",
            data: formdata,
            dataType: 'JSON',
            success: function(data) {
                $('#tbl_self_competency').DataTable().ajax.reload(null, false);
                $('#tbl_self_competency_approval').DataTable().ajax.reload(null, false);
                $('#form_approval_self_competency')[0].reset(); // reset form on modals
                $('#modal_form_approval_self_competency').modal('hide');
                alert('คุณทำการอนุมัติเรียบร้อยแล้ว');

                $('#btn_approval').text('ยืนยัน'); //change button text
                $('#btn_approval').attr('disabled', false); //set button enable
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert(messenger);
            }
